...
DROP TRIGGER IF EXISTS enforce_audit_log_user_id_trigger ON public.security_audit_log;
...
DROP TRIGGER IF EXISTS validate_audit_log_entry_trigger ON public.security_audit_log;
...
-- 3. ADD MISSING TRIGGERS FOR EXISTING SECURITY FUNCTIONS (WITH CHECKS)

-- Trigger for enhanced_session_tracking suspicious activity detection
DROP TRIGGER IF EXISTS detect_suspicious_session_activity_trigger ON public.enhanced_session_tracking;
...
-- Skip cms_media trigger if it already exists (keep existing one)
DO $$
BEGIN
  IF NOT EXISTS (
...
DROP TRIGGER IF EXISTS validate_csp_violation_trigger ON public.csp_violation_reports;
...
DROP TRIGGER IF EXISTS enforce_security_event_user_id_trigger ON public.security_events_enhanced;
...
DROP TRIGGER IF EXISTS enforce_comprehensive_log_user_id_trigger ON public.comprehensive_security_log;
