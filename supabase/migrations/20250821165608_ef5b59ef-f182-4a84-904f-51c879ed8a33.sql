-- SECURITY FIX: Restrict cms_media access to prevent unauthorized file deletion
...
CREATE POLICY "Users can update own media, admins all"
ON public.cms_media
FOR UPDATE
TO authenticated
USING (
...
CREATE POLICY "Users can delete own media, admins all"
ON public.cms_media
FOR DELETE
TO authenticated
USING (
...
BEGIN
  ...
  IF OLD.uploaded_by IS NOT NULL AND NEW.uploaded_by != OLD.uploaded_by THEN
...
INSERT INTO public.comprehensive_security_log (
  ...
  jsonb_build_object(
    ...
    'policies_created', array[
      'anyone_can_view_media',
      'users_upload_own_media',
      'users_update_own_admins_all',
      'users_delete_own_admins_all'
    ]
  )
);